...
# ============================================================
# AUTO-UPDATING PLATFORM DATABASE
# ============================================================
...
def check_platform_thread(name, data, username):
    ...
    try:
        if error_type == "status_code":
            if resp.status_code == error_code: return None
        elif error_type == "message":
            if error_msg and error_msg in resp.text: return None
        elif error_type == "response_url":
            if error_msg and error_msg in resp.url: return None
    ...
    if resp and resp.status_code == 200:
        try:
            ...
            if subdomains:
                ...
                for sub in list(subdomains)[:15]: log_print(f"     {Fore.YELLOW}- {sub}")
            else:
...
def check_hudson_rock(target):
    if is_email(target):
        url = f"https://cavalier.hudsonrock.com/api/json/v2/osint-tools/emails?email={target}"
    else:
        url = f"https://cavalier.hudsonrock.com/api/json/v2/osint-tools/usernames?username={target}"
    
    resp = robust_http_get(url, timeout=10, retries=1)
    if resp and resp.status_code == 200:
        try:
            data = resp.json()
            stealers = data.get("stealers", [])
            if stealers:
                log_print(f"  {Fore.RED}[⚠️]{Fore.WHITE} [HudsonRock] {Fore.CYAN}[Malware]{Fore.WHITE} -> Found in {len(stealers)} infection(s)!")
        except: pass

def check_free_credentials(target):
    print_section("🩸 PASSWORD SEARCH (FREE)")
    log_print(f"  {Fore.YELLOW}  [ℹ️] Commercial sites hide passwords.")
    log_print(f"  {Fore.YELLOW}      Open these links in the browser to see the password for free:\n")
...
def investigate_leaks(target, target_type):
    print_section(f"🔥 THREATS & DARK WEB: {target}")
    found = []
    steps = 4
        ...
        try:
            for item in pb.json()[:3]:
                ...
                if pid:
                    ...
                    raw_resp = robust_http_get(raw_url, timeout=5, retries=1)
                    snippet = "No access"
                    if raw_resp and raw_resp.status_code == 200 and target in raw_resp.text:
                        idx = raw_resp.text.find(target)
                        start = max(0, idx - 30)
                        end = min(len(raw_resp.text), idx + 50)
                        snippet = raw_resp.text[start:end].replace('\n', ' ').replace('\r', '').strip()
                    found.append(f"  {Fore.RED}[⚠️]{Fore.WHITE} [{formatted_date}] [Pastebin] {Fore.CYAN}[{target_type}]{Fore.WHITE} -> {Fore.YELLOW}https://pastebin.com/{pid} | Snippet: '{snippet}'")
        except: pass

    print_progress(3, steps)

    check_hudson_rock(target)

    print_progress(4, steps)

    if found:
        log_print(f"\n{Fore.RED}[!] LEAKS FOUND:\n")
        for s in found: log_print(s)
    else:
            ...
            if num.isdigit() and 1 <= int(num) <= len(reports):
                ...
                if action == '1':
                    ...
                    if input(f"{Fore.RED}Delete '{file}'? (y/n): {Fore.WHITE}").lower() == 'y':
                        try: os.remove(file); print(f"{Fore.GREEN}[+] Deleted!")
                        except: pass
                    input(f"\n{Fore.CYAN}Enter...")
            else:
...
def show_about():
    ...
    about_lines = [
        ...
        f"{Fore.GREEN}-{Fore.WHITE} Leaks: Pastebin, GitHub, HudsonRock",
        f"{Fore.GREEN}-{Fore.WHITE} Passwords: HIBP, BreachDirectory",
...
def save_report(target):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for l in report_buffer: f.write(l + "\n")
...
def investigate_all(target):
    ...
    if is_email(target):
        investigate_email(target); target_type = "Email"
    elif is_domain(target):
        investigate_domain(target); target_type = "Domain"
    elif is_phone(target):
        investigate_phone(target); target_type = "Phone"
    else:
        investigate_username(target); target_type = "Username/Password"
...
def main():
    ...
    while True:
        ...
        menu_lines = [
            ...
            f"",
            f"{Fore.GREEN}1.{Fore.WHITE} Investigate (Email/User/Phone/Pass)",
            f"{Fore.BLUE}2.{Fore.WHITE} Manage Reports",
            f"{Fore.MAGENTA}3.{Fore.WHITE} About",
            f"{Fore.CYAN}4.{Fore.WHITE} Update Platform Database",
        ...
        if choice == '1':
            ...
            if target: investigate_all(target)
            input(f"\n{Fore.CYAN}Enter...")
        elif choice == '2':
            manage_reports()
        elif choice == '3':
            ...
            clear_screen()
            load_or_update_platforms(force=True)
